...
-------------------------------
DBM = {
	Revision = tonumber(("$Revision: 15721 $"):sub(12, -3)),
	DisplayVersion = "7.1.11 bigfoot", -- the string that is shown as version			--bf@178.com
		...
		for i, v in ipairs(sortMe) do
			...
			if v.displayVersion and not v.bwversion then--DBM, no BigWigs
				if self.Options.ShowAllVersions then
					-- self:AddMsg(DBM_CORE_VERSIONCHECK_ENTRY:format(name, "DBM "..v.displayVersion, "r"..v.revision, v.VPVersion or ""), false)--Only display VP version if not running two mods	--bf@178.com
				end
			...
			elseif self.Options.ShowAllVersions and v.displayVersion and v.bwversion then--DBM & BigWigs
				-- self:AddMsg(DBM_CORE_VERSIONCHECK_ENTRY_TWO:format(name, "DBM "..v.displayVersion, "r"..v.revision, DBM_BIG_WIGS, versionResponseString:format(v.bwversion, v.bwhash)), false)	--bf@178.com
			elseif self.Options.ShowAllVersions and not v.displayVersion and v.bwversion then--BigWigs, No DBM
				-- self:AddMsg(DBM_CORE_VERSIONCHECK_ENTRY:format(name, DBM_BIG_WIGS, versionResponseString:format(v.bwversion, v.bwhash), ""), false)	--bf@178.com
			else
				if self.Options.ShowAllVersions then
					-- self:AddMsg(DBM_CORE_VERSIONCHECK_ENTRY_NO_DBM:format(name), false)					--bf@178.com
				end
			end
		end
			...
			if DBM.DisplayVersion:find("alpha") and #newerVersionPerson < 2 and #newerRevisionPerson < 2 and updateNotificationDisplayed < 2 and (revision - DBM.Revision) > 20 then
				...
				if #newerRevisionPerson == 2 then
					...
					if testBuild and revDifference > 5 then
						...
						updateNotificationDisplayed = 2
						-- DBM:AddMsg(DBM_CORE_UPDATEREMINDER_HEADER_ALPHA:format(revDifference))	--bf@178.com
					end
				end
			end
		end
		...
		if removeEntry(inCombat, mod) then
			...
			if wipe then
				...
				if showConstantReminder == 2 and IsInGroup() and savedDifficulty ~= "lfr" and savedDifficulty ~= "lfr25" then
					...
					-- self:AddMsg(DBM_CORE_OUT_OF_DATE_NAG)	--bf@178.com
				end
				local msg
...
do
	...
	function bossModPrototype:NewSound(spellId, optionDefault, optionName)
		if not (optionName == false) then--Basically, all mods still using NewSound.
			...
			-- print("Error, NewSound depricated. Update your old DBM mods to remove this error")		--bf@178.com
			return
		end
