...
function MarkAssistantConifgFunc()
	...
	elseif (GetLocale() == "zhTW") then
		...
		TargetCharms_ENABLE_TEXT = "Enable TargetCharms"
		TargetCharms_ENABLE_TOOLTIP = "Enable this option later you can quickly placed column mark, when you're a captain also can be use team mark"
		TargetCharms_Set_Text = "Set"

		EZICON_ENABLE_TEXT="Enable ezIcon"
		EZICON_ENABLE_TOOLTIP = "ezIcon allows you to double click on a target and set a raid icon。"
	end

	if (IsConfigurableAddOn("ezIcons") or IsConfigurableAddOn("TargetCharms")) then
	...
	local NewInfoStr =[[
	之前我们添加了标记助手的功能，但是设置起来很麻烦，现在我们为标记助手提供了独立的开关，方便玩家设置。

	功能所在位置：
	大脚标记-开启标记助手
	]]
	if (IsConfigurableAddOn("TargetCharms")) then
		ModManagement_RegisterCheckBox(
			"MarkAssistant",
			TargetCharms_ENABLE_TEXT,
			TargetCharms_ENABLE_TOOLTIP,
			"EnableTargetCharms",
			0,
			function (__arg)
				if (__arg == 1) then
